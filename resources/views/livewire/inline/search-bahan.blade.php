    ...
    <div class="w-1/2 flex items-center relative">
        ...
        <div class="relative w-full">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg aria-hidden="true" class="w-5 h-5 text-[#3E3E3E] dark:text-gray-400" fill="#3E3E3E" viewBox="0 0 20 20"
            ...
            <div wire:loading
                class="absolute bg-white z-10 text-base font-medium text-gray-800 list-group w-full rounded-b-lg shadow-lg px-4 py-3">
                <li class="list-none">
            ...
            @if ($kategoriBar)
                <div class="overflow-y-scroll h-full max-h-screen">
                    <div
                        class="absolute bg-white z-10 text-base font-medium text-gray-800 list-group w-full rounded-b-lg shadow-lg">
                        @if (!empty($listBahan))
                            @foreach ($listBahan as $item)
                                <ul class="list-group">
                                    <li class="list-group-item hover:bg-blue-100 hover:rounded-b-lg px-4 py-2 cursor-pointer"
                                        wire:click="selectBahan('{{ $item['id'] }}')">
                    ...
                    <div
                        ...
                        @if (!empty($bahan))
                            @foreach ($bahan as $item)
                                <ul class="list-group">
                                    <li class="list-group-item hover:bg-blue-100 hover:rounded-b-lg px-4 py-2 cursor-pointer"
                                        wire:click="toggleTag('{{ $item['nama_bahan'] }}')"
                                        wire:click="showResep('{{ $item['id'] }}')">
                                        <div class="inline-flex items-center">
                                            <img src="storage/images/bahan/{{ $item['image'] }}" alt=""
                                                class="w-[50px] h-[50px] rounded-lg">
                                            <span class="pl-3">{{ $item['nama_bahan'] }}</span>
        ...
        @if (!empty($selectedBahan))
            @foreach ($selectedBahan as $tag)
                ...
                <button id="tag"
                    ...
                    {{ $tag['nama_bahan'] }}
                    <svg xmlns="http://www.w3.org/2000/svg" wire:click="deleteTag('{{ $tag['id'] }}')"
